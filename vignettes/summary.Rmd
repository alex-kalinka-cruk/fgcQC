...
If `mock_missing_data` is set to `TRUE` (defaults to `FALSE`) then one or both of `bcl2fastq` and `library` can be set to `NULL` and the missing data will be mocked within the function and the mocked columns will be set to `NA` values in the output. When this is the case, a mock 'qc' section will also be included in the analysis config.
