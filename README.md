...
metrics; both standard and novel metrics are included.
...
-   The main entry point is `QC_fgc_crispr_data()` and this function
    ...
    sequencing metrics (bcl2fastq), and a gRNA library file.
...
provided in a single comma-separated string. For lethality screens, set
`bagel_treat_plasmid` to `NULL`.
...
QC metrics to the left of the `SampleId` column are comparison-wide, or
for pairs of samples (e.g. control vs plasmid). QC metrics to the right
of the `SampleId` column are sample-specific metrics. In the below
tables, `<comp>` is one of `ctrl_plasmid`, `treat_plasmid`, or
`treat_ctrl`, and `<gene_set>` is the name of a gene set, e.g.
`hart_essential`.
...
<td>Mahalanobis distance ratio of control or treatment samples relative to the plasmid. Lower values indicate the control or treatment samples cluster away from the plasmid. Values <span class="math inline">≥</span> 1 indicate control or treatment samples are close to the plasmid and should be investigated.</td>
</tr>
<tr class="odd">
...
<td>The percentage of reads that match at least 1 gRNA in each sample.</td>
</tr>
<tr class="odd">
...
<td>The percentage of 1-base mismatches per sample. Should be low.</td>
</tr>
</tbody>
</table>
